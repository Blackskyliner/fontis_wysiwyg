<?php
/// --- Magento Patch Begin
require_once(__DIR__.'/adapter-mage.php');
$adapterConfiguration = adapter_magento();

if(!$adapterConfiguration['enabled']){
    echo "File uploads disabled.";
    exit;
}

if(!$adapterConfiguration['logged_in']){
    echo "<script>alert('Admin session doesn\'t exist, please login')</script>";
    exit;
}

$_SESSION['username'] = $adapterConfiguration['logged_in_as'];
...
$useruploadroot = "//".$_SERVER['HTTP_HOST'];
...
if ($adapterConfiguration) {
    $useruploadfolder = $adapterConfiguration['upload_dir_public'];

    // full magento path
    $useruploadpath = $adapterConfiguration['upload_dir'];

    $foldershistory = array($useruploadfolder);
}
